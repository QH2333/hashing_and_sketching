...
void monitor_live(int max_pkt_cnt, topk_algo_base* algo_obj)
{
    ...
    pcap_if_t *iter = alldevsp;
    for (int i = 1; i < selected_id; i++)
    {
        iter = iter->next;
    }
    strncpy(devnamebuf, iter->name, 255);
    pcap_freealldevs(alldevsp);

    printf("Capturing %s.\n", devnamebuf);

    // Create a pcap session
    pcap_t *pcap_session = pcap_open_live(devnamebuf, 65535, 1, 500, errbuf);
